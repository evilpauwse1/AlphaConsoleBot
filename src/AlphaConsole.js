...
client.on("userUpdate", (oldMember, newMember) => {
  if (oldMember.tag !== newMember.tag)
    client.guilds.get(serverInfo.guildId).channels.get(serverInfo.channels.serverlog).send(`:person_with_pouting_face: \`[${new Date().toTimeString().split(" ")[0]}]\` **\`${oldMember.tag}\` (\`${oldMember.id}\`)** changed their Discord name to **\`${newMember.tag}\`**`)
